...
args.option('input', 'Path to input text')
args.option('output', '(Partial) name of output file')
args.option('split-sentences', 'Whether to split sentences up', false)

const { input, output, splitSentences } = args.parse(process.argv)

// prediction params
let sample_softmax_temperature = 0.7 // how peaky model predictions should be
...
function run() {
  let book = fs.readFileSync(path.resolve(input), 'utf8')
